...
public:
	...
	int neutral;
	int difficulty;
	int sost; // 0 - nothing 1- battle 2-dialog
...
{
	string path = "Files\\levels\\\\loc.txt";
	path.insert(13, to_string(level));
	path.insert(17 + to_string(level).length(), to_string(loc));
	gotoxy(0, 0);
	loadLevelBase(path);
	player.loc = loc;
	printCharS();
...
/// Получение предмета
/// </summary>
/// <param name="x"> место в инвентаре или снаряжении 
/// в инвентаре если 0 оружие 1 голова 2 плечи 3 нагрудник 4 перчатки 5 поножи 6 сапоги</param>
/// <param name="y">1 снаряжение или 2 инвентарь</param>
/// <param name="id">id получаемого предмета</param>
void GETItem(int x, int y, int id)//x - место в инвентаре или снаряжении y - 1/снаряжение 2/инвентарь
{
	...
	if (player.Class == "Лучник")
		f = 3;
	string fileName = "Files\\Items\\" + to_string(id) + to_string(f) + ".txt";
	ifstream openfile(fileName.c_str());
	getline(openfile, getContent);
...
void printItem(int x, int y, int id)
{
	...
	string getContent;
	string fileName = "Files\\Items\\" + to_string(id) + ".txt";
	ifstream openfile(fileName.c_str());
	SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 15);
	if (openfile.is_open())
	{
...
{
	string path = "Files\\music\\"+name + ".wav";
	wstring widestr = wstring(path.begin(), path.end());
	const wchar_t* widecstr = widestr.c_str();
	PlaySound(widecstr, NULL, SND_LOOP || SND_ASYNC);
}
void SoundBeep(string name)
{
	string path = "play Files\\sounds\\" + name + ".wav";
	wstring widestr = wstring(path.begin(), path.end());
	const wchar_t* widecstr = widestr.c_str();
...
void printOsn()
{
	...
	if (player.Class == "Лучник")
		f = 3;
	string fileName = "Files\\Items\\" + to_string(player.equipment[1].id) + to_string(f) + ".txt";
	...
	setTextBack();
	fileName = "Files\\Items\\" + to_string(player.equipment[2].id) + to_string(f) + ".txt";
	...
	setTextBack();
	fileName = "Files\\Items\\" + to_string(player.equipment[3].id) + to_string(f) + ".txt";
	...
	setTextBack();
	fileName = "Files\\Items\\" + to_string(player.equipment[4].id) + to_string(f) + ".txt";
	...
	setTextBack();
	fileName = "Files\\Items\\" + to_string(player.equipment[5].id) + to_string(f) + ".txt";
	...
	setTextBack();
	fileName = "Files\\Items\\" + to_string(player.equipment[0].id) + to_string(f) + ".txt";
	ifstream openfile(fileName.c_str());
	getline(openfile, getContent);
	SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 15);
	if (openfile.is_open())
...
void printAboutChar()
{
	...
	cout << "|      I - Открыть инвентарь    |       C - О персонаже       |     J - Открыть журнал заданий     |      M - открыть карту республики      |        H - Открыть окно помощи    |        ESC - Открыть меню       |";
...
void openWorldMap()
{
	...
	cout << "        Нажмите M чтобы закрыть карту    ";
	char f;
	while (1)
	{
		f = _getch();
		if (f == 'M' || f == 'm' || f == -100 || f == -20)
		{
...
void DialogOutput(string str, int y)
{
	switch (str[0])
	{
	case '1':
...
void dialog(int x)
{
	...
	else
	{
		...
		string fileName = "Files\\CharactersForDialogs\\" + str + ".txt";
		int yy = 0;
		{ifstream openfile(fileName.c_str());
		SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 15);
		if (openfile.is_open())
		...
		string fileName1 = "Files\\Dialogs\\" + to_string(x) + ".txt";
		ifstream openfile(fileName1.c_str());
		yy = 0;
		if (openfile.is_open())
		{
			while (!openfile.eof())
			{
				if (getContent[0] == '1'|| getContent[0] == '2')
					yy++;
				getline(openfile, getContent);
				
				DialogOutput(getContent, yy);
						...
						if (wAttrib == 'K')
						{
							...
							case 0:
								...
								case 1:
									...
									//mciSendString(L"play 2.wav", NULL, 0, NULL);
...
char nearSymb(char x)
{
	...
	case 'i':
		ReadConsoleOutputCharacter(GetStdHandle(STD_OUTPUT_HANDLE), &wAttrib, 1, coord, &dwReaden);
		return conditionsForObjects(wAttrib, 'i');
...
void Calibration()
{
	system("cls");
	gotoxy(20, 20);
	cout << "Калибровка экрана";
	gotoxy(25, 21);
	cout << "С помощью CTRL+Колесо мыши откалибрируйте масштаб так, чтобы был виден лишь один из двух углов в правом углу экрана...";
	gotoxy(27, 23);
	cout << "Для продолжения нажмите W";
	bool flag = 1;
	gotoxy(207, 53);
	cout << "____";
	gotoxy(210, 53);
...
void OpenAboutChar()
{
	...
	gotoxy(85, 53); cout << " Нажмите C чтобы закрыть окно с информацией о персонаже    ";
	bool flag = 1;
	while (flag)
	{
		char f = _getch();
...
void functionForInitializationObjects(char Char, int color, int level, int loc, int x, int y, bool IsAlive)
{
	Objects[count_obj].Char = Char;//создать функцию для инициализации объектов
	Objects[count_obj].color = color;
	Objects[count_obj].level = level;
	Objects[count_obj].loc = loc;
	Objects[count_obj].x = x;
	Objects[count_obj].y = y;
	Objects[count_obj].IsAlive = IsAlive;
	count_obj++;
}
void setStartPrisoners()
{
	functionForInitializationObjects('K', 3, 1, 0, 12, 14, true);
	functionForInitializationObjects('P', 1, 1, 0, 15, 4, true);
	functionForInitializationObjects('P', 1, 1, 0, 25, 5, true);
	functionForInitializationObjects('g', 3, 1, 0, 19, 9, true);
	functionForInitializationObjects('g', 3, 1, 0, 16, 9, true);
	functionForInitializationObjects('g', 3, 1, 0, 19, 22, true);
	functionForInitializationObjects('P', 1, 1, 0, 25, 5, true);
...
void Start()
{
	...
	gotoxy(0, 0);
	loadLevelBase("Files\\levels\\Start\\start.txt");
	player.loc == 0;
	startPoint(87, 5);
	gotoxy(87, 5);
	setTextRed();
...
int main()
{
	...
	fontInfo.dwFontSize.X = 20; // Размер (в логических единицах)
